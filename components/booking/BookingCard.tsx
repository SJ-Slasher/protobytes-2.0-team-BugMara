...
import { cn, formatDuration, getConnectorLabel, getBookingStatusColor } from "@/lib/utils";
...
export function BookingCard({ booking, onCancel, showActions = true }: BookingCardProps) {
  // Validate station data - only use if it has required fields
  const station = typeof booking.stationId === "object" && booking.stationId && "name" in booking.stationId
  ...
  // Fallback for when date parsing fails
  let startDate: Date;
  try {
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-card-foreground">
              {station?.name ? station.name : `Charging Station (ID: ${String(booking.stationId).slice(0, 8)}...)`}
            </h3>
          ...
          {station && station.location && (
            <div className="mt-1 flex items-center gap-1 text-sm text-muted-foreground">
              ...
              <span>
                {station.location.city || "Location"}
